            ...
            if (path.get('callee').isIdentifier()) {
                ...
                if (isDecodeFunc && path.get('arguments').length === 1) {
                    ...
                    if (types.isNumericLiteral(arg)) {
                        ...
                        // 如果是成员表达式（如 obj.prop），从variableValues中获取值
                        const objName = arg.object.name;
                        const propName = arg.property.name;
                        if (variableValues[objName] && variableValues[objName][propName] !== undefined) {
                            argValue = variableValues[objName][propName];
                        }
                    }
